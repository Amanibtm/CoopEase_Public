...
def generate_schedule_for_departments(departments, semester=None):
    try:
        # TIME
        time_days = TimeSlot.objects.filter(departements__in=departments).distinct()
        ...
        days = sorted(days, key=lambda day: day_order.get(day, 99)) #99 is a default value if the day in days is not recognized
        ...
        # CLASSROOMS
        Spaces = Space.objects.filter(departements__in=departments)
        spaces = {}
        for each in Spaces:
        ...
        Groups = Group.objects.filter(speciality__level__departement__in=departments)
                ...
                for i in range(sections):
                    ...
                    for group in groupslist[start:end]:
                        try:
                            ...
                            print(f"groupe {group.group_number} not found in DB... check data!")
        ...
        for speciality, sections in groups.items():
            for section_name, group_list in sections.items():
                if (speciality.level.departement.est.SystemeEtudeEnGroupe == "Groupes-Tous"
        ...
        if semester in ["s1","s2"]:
            existing_prof_time_slots = set(Schedule.objects.filter(speciality__level__departement__in=departments, semester=semester, school_year=academic_year, is_activated=True).values_list(
                'professor', 'day', 'time'))
        else:
            existing_prof_time_slots = set(
                Schedule.objects.filter(speciality__level__departement__in=departments, school_year=academic_year, is_activated=True).values_list(
                    'professor', 'day', 'time'))


        pending_saves = []

        for speciality in Speciality.objects.filter(level__departement__in=departments, is_activated=True):
            ...
            for section_name in sections:
                ...
                if has_tous:
                    ...
                    indispos=busyTimeSelect.objects.filter(user__modules__specialityMod__level__departement__in=departments, is_approved=True,
                                                           timestamp__date__range=(start_date, end_date))
                    prof_busy_times = set()
                    for entry in indispos:
                        prof = AdvancedUserProfile.objects.get(first_name=entry.user.first_name,last_name=entry.user.last_name)
                    ...
                    #5. avoid conflicts with shared rooms existing schedules
                    ...
                    busy_shared_slots = Schedule.objects.filter(
                        ...
                        is_activated=True,school_year=academic_year,semester=semester).exclude(speciality__level__departement__in=departments).values('room_number','room_type','day','time')

                    # Convert to a set for fast lookup
                    busy_shared_set = {(b['room_type'], b['room_number'], b['day'], b['time']) for b in busy_shared_slots}
                    #print('busy shared slots : \n',busy_shared_set)

                    for key, var in cours_tous_assignments.items():
                        # Avoid shared room if it's busy elsewhere
                    ...
                    model.Maximize(sum(cours_tous_assignments.values()) * 1000 +  # Primary objective (ensure all scheduled)
                                   sum(time_preference_terms))  # Secondary objective (time preference)

                    solver = cp_model.CpSolver()
                    solver.parameters.num_search_workers = 8 # Use 8 CPU threads to speed up the solving process
                ...
                for module in modules:
                    content = {}
                    if module.TD: content["TD"] = module.TD.name
                    elif not module.TD and module.nbrTDSemaine not in [None, 0]: content["TD"] = "Enligne"

                    if module.TP: content["TP"] = module.TP.name
                    elif not module.TP and module.nbrTPSemaine not in [None, 0]: content["TP"] = "Enligne"

                    if module.Cours: content["Cours"] = module.Cours.name
                    elif not module.Cours and module.nbrCoursSemaine not in [None, 0]: content["Cours"]="Enligne"
                ...
                #7. avoid conflicts with shared rooms existing schedules
                shared_spaces = Space.objects.annotate(num_depts=Count('departements')).filter(num_depts__gt=1).distinct()
                # Get schedules in those shared rooms made by other departments
                busy_shared_slots = Schedule.objects.filter(
                    room_number__in=[shared_space.room_number for shared_space in shared_spaces],
                    room_type__in=[shared_space.room_type.name for shared_space in shared_spaces],
                    is_activated=True, semester=semester, school_year=academic_year).exclude(speciality__level__departement__in=departments).values('room_number', 'room_type', 'day', 'time')
        ...
        for entry in pending_saves:
            print(entry)
        # Save to database
        with transaction.atomic(): #If one fails, nothing is saved
            Schedule.objects.filter(speciality__level__departement__in=departments,is_activated=True,semester=semester, school_year=academic_year).delete()

            for entry in pending_saves:
                try:
                    module = Module.objects.get(specialityMod=entry['speciality'],module_name=entry['module_name'],is_activated=True,semester=semester)
